...
namespace MS.Shell.Editor.Tests{
    ...
    {
        private static EditorShellTests test; 
        ...
        public static void Test()
        {
            if (test == null)
            {
                test = new EditorShellTests();
            }

            Debug.Log("---Start shell---");
            var task = EditorShell.Execute(Application.dataPath+ "/EditorShell/Test.bat",new EditorShell.Options());
            task.onLog += (logType,log)=>{
                Debug.Log(log);
            };
            task.onExit += (code)=>{
                Debug.Log("---Shell exit with code = " + code);
                Assert.True(code == 0);
            };
        }
